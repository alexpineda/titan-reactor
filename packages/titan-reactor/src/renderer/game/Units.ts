...
import BuildingQueueCountBW, {
  TrainingQueueType,
  BuildingQueueI,
} from "../game-data/BuildingQueueCountBW";
import { PxToGameUnit } from "../../common/types/util";
import FogOfWar from "../fogofwar/FogOfWar";
import UnitsBW, { UnitBWI } from "../game-data/UnitsBW";
...
class Units {
  ...
  private resourceImageData: ImageData;
  ...
  _refreshMinimap(unit: GameUnitI) {
  ...
  refresh(
    ...
    unitsInProduction: GameUnitI[]
  ) {
    this.imageData.data.fill(0);
    this.resourceImageData.data.fill(0);

    const incompleteUnits: Map<number, GameUnitI> = new Map();

    for (const unitBw of unitsBW.instances() as UnitBWI[]) {
      let unit: GameUnitI;
