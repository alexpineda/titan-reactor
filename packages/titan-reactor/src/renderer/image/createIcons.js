...
import {
  rgbToCanvas,
  canvasToHtmlImage,
} from "titan-reactor-shared/image/canvas";
...
export const createIcons = (buf, palette) => {
  ...
  const sdIcons = range(0, grp.frameCount()).map((frame) => {
    ...
    return canvasToHtmlImage(
      rgbToCanvas({ data, width: w, height: h }, "rgba")
    );
  });
...
export const createWireframes = (buf, palette) => {
  ...
  const wireframeSD = new WireframeSD(
    rgbToCanvas({ data: texData, width: cw, height: ch }, "rgba"),
    frames
  );
