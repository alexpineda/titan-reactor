...
const displayTextSelector = (state: UnitSelectionStore) => {
  if (!state.selectedUnits[0]) return "";
  return getDisplayText(state.selectedUnits[0]);
};

const researchSelector = (state: UnitSelectionStore) => {
...
const Progress = forwardRef(
  ({ unit }: Props, ref: React.RefObject<HTMLElement>) => {
    ...
    const progressSelector = (state: UnitSelectionStore) => {
      if (!state.selectedUnits[0]) return 0;
    ...
    const selector = (state: UnitSelectionStore): [number, string] => {
      return [
