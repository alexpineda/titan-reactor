export default (THREE) => {
  ...
  DrawCallInspector.prototype = {
    ...
    mount: function (element = document.body) {
      const { skipFrames, useMaterials, renderer } = this;
      ...
      const classes = [THREE.Mesh, THREE.SkinnedMesh, THREE.Line];

      if (THREE.InstancedMesh) classes.push(THREE.InstancedMesh);
      ...
      for (let cls of classes) {
        ...
        Object.defineProperty(cls.prototype, "isMesh", {
          get: function () {
            ...
            if (enableMeasure === 1) enableMeasure++;
      ...
      renderer.renderBufferDirect = function (
        ...
        if (self.stage === 1) {
          ...
          if (useMaterials) MaterialIndex.push(object, geometry, material);
          ...
          if (object.userData.delta !== undefined) {
            object.userData.age *= 0.9;
            object.userData.delta +=
              (object.userData.deltaTime / self.maxDelta -
                object.userData.delta) *
              self.fade;
    ...
    begin: function () {
      if (this.mounted) {
        ...
        if (this.skipFrames > -1) this.frameIndex++;

        if (
          this.needsRecord ||
          (this.frameIndex > this.skipFrames && this.skipFrames > -1)
        ) {
    ...
    update: function () {
      if (this.needsUpdate) {
        ...
        const { renderer, resolution, ctx } = this;
