...
export default class Chk {
  ...
  static fsFileAccess(directory) {
    const cb = (fname) =>
      new Promise((res, rej) => {
        fs.createReadStream(directory + "/" + fname.replace(/\\/g, "/")).pipe(
  ...
  async image(fileAccess, width, height, options) {
    ...
    this._renderTerrain(tileset, out, width, height, opts);
  ...
  _processMapPixels(tileset, out, width, height, { colorAtMega }) {
    if (this.size[0] !== width / 32 || this.size[1] !== height / 32) {
      throw new Error("Image size must be 32x per map dimension");
    }

    const mapWidth = this.size[0];
    const mapHeight = this.size[1];

    for (let mapY = 0; mapY < mapHeight; mapY++) {
      if (this.onProcess) {
        this.onProcess(mapY / mapHeight);
      }
      for (let mapX = 0; mapX < mapWidth; mapX++) {
        const mapTile = mapY * mapWidth + mapX;

        let mapTileId = 0;
        if (mapTile * 2 + 2 > this._tiles.length) {
          mapTileId = 0;
        } else {
          mapTileId = this._tiles.readUInt16LE(mapTile * 2);
        }

        const tileGroup = mapTileId >> 4;
        const groupIndex = mapTileId & 0xf;
        ...
        const groupMeta = {
          unknown: tileset.tilegroup.readUInt8(tileGroup * 0x34 + 2) & 0xf,
          buildable: tileset.tilegroup.readUInt8(tileGroup * 0x34 + 2) & 0x0f,
          groundHeight: tileset.tilegroup.readUInt8(tileGroup * 0x34 + 3) & 0xf,
          unknown2: tileset.tilegroup.readUInt8(tileGroup * 0x34 + 3) & 0x0f,
          leftEdge: tileset.tilegroup.readUInt16LE(tileGroup * 0x34 + 4),
          topEdge: tileset.tilegroup.readUInt16LE(tileGroup * 0x34 + 6),
          rightEdge: tileset.tilegroup.readUInt16LE(tileGroup * 0x34 + 8),
          bottomEdge: tileset.tilegroup.readUInt16LE(tileGroup * 0x34 + 10),
          unknown3: tileset.tilegroup.readUInt16LE(tileGroup * 0x34 + 12),
          hasUp: tileset.tilegroup.readUInt16LE(tileGroup * 0x34 + 14),
          unknown4: tileset.tilegroup.readUInt16LE(tileGroup * 0x34 + 16),
          hasDown: tileset.tilegroup.readUInt16LE(tileGroup * 0x34 + 18),
        };

        for (let miniY = 0; miniY < 4; miniY++) {
          for (let miniX = 0; miniX < 4; miniX++) {
            const mini = tileset.megatiles.readUInt32LE(
            ...
            minitile.low = !minitile.mid && !minitile.high ? 1 : 0;
            ...
            for (let py = 0; py < 8; py++) {
              for (let px = 0; px < 8; px++) {
                const p =
                  mapY * mapWidth * 32 * 32 +
                  mapX * 32 +
                  (miniY * mapWidth * 32 * 8 + miniX * 8) +
                  (py * mapWidth * 32 + px);

                const [r, g, b] = colorAtMega(
                  tileset,
                  megaTileId,
                  mapX * 32 + miniX * 8 + px,
                  mapY * 32 + miniY * 8 + py
  ...
  async getTileInfoFromXY(fileAccess, width, height, px, py) {
    const tileset = await fileAccess.tileset(this.tileset);
    return this._getTileInfoFromXY(tileset, width, height, px, py);
  }

  _getTileInfoFromXY(tileset, width, px, py) {
    const mapWidth = width / 32;
    const mapX = Math.floor(px / 32);
    const mapY = Math.floor(py / 32);
    const miniX = Math.floor((px % 32) / 8);
    const miniY = Math.floor((py % 32) / 8);
  ...
  _renderTerrain(tileset, out, width, height, opts) {
    ...
    const megatiles = generateScaledMegatiles(tileset, pixelsPerMega, opts);
...
function generateScaledMegatiles(tileset, pixelsPerMega, { colorAtMega }) {
  const megatileCount =
