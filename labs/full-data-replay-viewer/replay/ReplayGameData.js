...
export default class ReplayGameData {
  static fromScrepJSON({ Header, Computed, MapData, Commands }) {
    return {
  ...
  static fromJssuhJSON({ header, commands, mapData }) {
    ...
    const data = {
      ...
      commands: commands.map(({ frame, id, player, data }) => {
        ...
        return { ...cmd, ...ParseCmd.toJson(cmd, data) }
      }),
