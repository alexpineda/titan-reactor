declare module "titan-reactor/host" {
    ...
    export type SceneInputHandler = NativePlugin &
        Partial<UserInputCallbacks> & {
            dispose: () => void;
            gameOptions: { allowUnitSelection: boolean; audio: "stereo" | "3d" };
            onEnterScene: ( prevData: any ) => Promise<void>;
            onExitScene?: ( ( currentData: any ) => any ) | undefined;
            onPostProcessingBundle: (
                renderPass: any,
                fogOfWarEffect: any
            ) => { passes?: any[] | undefined; effects?: any[] | undefined };
        };
    ...
    export type PluginStateMessage = {
        ...
        screen: { screen: SceneStateID | undefined; error: string | undefined };
        world: {
            map:
                | {
                      title: string;
    ...
    export type SceneStateID =
        ...
        | "@iscriptah"
        | "@interstitial";
    export type UIStateAssets = {
        bwDat: BwDAT;
        gameIcons: ResourceIcons;
        cmdIcons: ( ArrayBuffer | string )[];
        raceInsetIcons: RaceInsetIcons;
        workerIcons: WorkerIcons;
        wireframeIcons: ( Blob | string )[];
    };
    export interface WorkerIcons {
        apm: ArrayBuffer | string;
        terran: ArrayBuffer | string;
        zerg: ArrayBuffer | string;
        protoss: ArrayBuffer | string;
    }
            ...
            if ( this.center ) {
                ...
                this.viewport.set(
                    MathUtils.clamp(
                        x,
                        this.#width / 2,
                        this.surfaceWidth - this.#width / 2
                    ),
                    MathUtils.clamp(
                        y,
                        this.height / 2,
                        this.surfaceHeight - this.height / 2
                    ),
                    this.width,
                    this.height
                );
    ...
    export type PxToGameUnit = {
        x: ( v: number ) => number;
        y: ( v: number ) => number;
        xy: ( x: number, y: number, out?: Vector2 | undefined ) => void;
        xyz: (
            x: number,
            y: number,
            out?: Vector3 | undefined,
            zFunction?: ( ( x: number, y: number ) => number ) | undefined
        ) => void;
    };
    ...
    export class Terrain extends Group {
        ...
        setAnisotropy( anisotropy: string ) {
            const value =
                anisotropyOptions[anisotropy as keyof typeof anisotropyOptions];
    ...
    export class Janitor {
        ...
        on(
            nodeEventListener: NodeJS.EventEmitter,
            event: string,
    ...
    export type Atlas = {
        ...
        frames: AnimFrame[];
        diffuse: Texture;
        teammask?: Texture | undefined;
        unitTileScale: number;
        grp: GrpSprite;
        hdLayers?: HDLayers | undefined;
        mipmap?: HDLayers | undefined;
    };
    ...
    export type SupportedJanitorTypes =
        | Disposable
        | EmptyFn
        | Object3D<Event>
        | EventEmitter;
    export type HDLayers = {
    ...
    export class OpenBWFileList {
        ...
        async loadBuffers(
            readFile: ( filename: string ) => Promise<Buffer | Uint8Array>
        ) {
            if ( this._cleared ) {
