...
export class Grp {
  ...
  decode(frame: number, palette: number[] | Uint8Array | Buffer, hasAlpha = false, isShadow = false) {
  ...
  render(frame: number, palette: number[], surface: number[], surfX: number, surfY: number, surfW: number, surfH: number, scaleX: number, scaleY: number) {
