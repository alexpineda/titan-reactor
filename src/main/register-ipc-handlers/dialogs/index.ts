...
ipcMain.handle( OPEN_REPLAY_DIALOG, async ( _, multiSelect = false ) =>
    showOpenReplayDialog( multiSelect )
);

ipcMain.handle( SHOW_FOLDER_DIALOG, () => showOpenFolderDialog() );

ipcMain.on( DOWNLOAD_UPDATE, ( _, url: string ) => {
    if ( url.startsWith( "https://github.com/imbateam-gg/titan-reactor/releases" ) ) {
        shell.openExternal( url, { activate: true } );
    }
} );

ipcMain.on( OPEN_URL, ( _, url: string ) => {
    const isWhitelisted =
