...
export class PluginManager {
    ...
    getCompatiblePlugins() {
        return this.#pluginPackages.filter(plugin => !this.#isIncompatible(plugin.apiVersion));
    }
    ...
    async #loadPluginPackage(folderPath: string, folderName: string): Promise<null | PluginMetaData> {
        ...
        if (await fileExists(path.join(folderPath, "plugin.ts"))) {
            ...
            if (tsSource) {
                try {
                    const result = transpile(tsSource, packageJSON.name, `${path.basename(folderPath)}.plugin.ts`, true);
    ...
    #isIncompatible(apiVersion: string) {
        return semver.major(packagejson.config["titan-reactor-api"]) !==
    ...
    async installPlugin(repository: string, onUpdated?: () => void) {
        ...
        try {
            ...
            try {
                const loadedPackage = await this.#loadPluginPackage(folderPath, folderName);

                if (loadedPackage) {
                    const pluginToUpdate = this.#pluginPackages.find(p => p.name === loadedPackage.name);
                    // we are not only installing but also updating this package
                    if (pluginToUpdate) {
                        const oldConfig = pluginToUpdate.config;
                        this.#pluginPackages.splice(this.#pluginPackages.indexOf(pluginToUpdate), 1, loadedPackage);
    ...
    async uninstallPlugin(pluginId: string) {
        ...
        try {
            ...
            this.#pluginPackages = this.#pluginPackages.filter(otherPlugin => otherPlugin.id !== pluginId);
        } catch {
