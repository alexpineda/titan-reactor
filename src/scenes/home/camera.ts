...
export const createCamera = () => {
    ...
    return {
        ...
        init( controls: CameraControls, battleCruiser: Object3D ) {
            ...
            const _zoom = setInterval( () => {
                if ( this.cameraState === CameraState.RotateAroundWraiths ) {
                    controls.zoomTo( Math.random() * 2 + 1.75 );
                }
            }, 20000 );
            janitor.add( () => clearInterval( _zoom ), "zoomToRandom" );

            let _timeout: NodeJS.Timeout;

            const evolveCameraState = setInterval( () => {
                _evolvingCameraState =
                    ++_evolvingCameraState % evolvingCameraStates.length;
                ...
                if ( this.cameraState === CameraState.UnderBattleCruiser ) {
                    controls.setLookAt(
                        ...
                        -1120,
                        -1040,
                        battleCruiser.position.x,
                        battleCruiser.position.y,
                        battleCruiser.position.z,
                        false
                    );
                    controls.zoomTo( 2 );
            ...
            janitor.add( () => clearInterval( evolveCameraState ), "evolveCameraState" );

            janitor.add( () => clearTimeout( _timeout ), "clearTimeout" );
        ...
        update(
            ...
            _polarAngle = MathUtils.damp(
                _polarAngle,
                _minPolarAngle +
                    Math.sin( delta / 1000 ) * _polarAngleRange * Math.sign( mouse.y ),
                0.001,
                delta
            );
