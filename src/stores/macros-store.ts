...
import { LocalStorageAdapter } from "@stores/storage-adapters/localstorage";
import { SettingsStore } from "./settings-store";
export interface MacroStore {
    macros: MacrosDTO;
    persist(): Promise<MacrosDTO | undefined>;
    init: () => void;
    busy: boolean;
...
export const createMacroStore = (
    ...
    create(
        immer<MacroStore>((set, get) => ({
            ...
            init() {
                const macros = storage.loadMacros();
                set((state) => {state.macros = macros});
            },
            ...
            async persist() {
                ...
                try {
                    const macros = await storage.saveMacros({
            ...
            createActionable({ id }: MacroDTO, action: MacroAction | MacroCondition) {
                const saneActionable = sanitizeActionable(
                    action,
                    settingsStore().data,
                    get().macros.macros,
            ...
            updateActionable({ id }: MacroDTO, _actionable: Actionable) {
                const actionable = sanitizeActionable(
                    _actionable,
                    settingsStore().data,
                    get().macros.macros,
                    pluginsStore().enabledPlugins
