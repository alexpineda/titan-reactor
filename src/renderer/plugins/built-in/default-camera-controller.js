...
export default class PluginAddon extends SceneController {
    ...
    async onEnterScene( prevData ) {
        ...
        if (
            typeof prevData?.target?.x === "number" &&
            typeof prevData?.target?.z === "number"
        ) {
    ...
    onCameraMouseUpdate(
        ...
        if ( screenDrag.y !== 0 ) {
            this.viewport.orbit.forward(
                screenDrag.y * _delta * this.settings.input.movementSpeed(),
                true
            );
        }
    }
    onCameraKeyboardUpdate( delta, elapsed, move ) {
        ...
        if ( move.x !== 0 ) {
            this.viewport.orbit.truck(
                move.x * _delta * this.settings.input.movementSpeed(),
                0,
                true
            );
        }
        if ( move.y !== 0 ) {
            this.viewport.orbit.forward(
                move.y * _delta * this.settings.input.movementSpeed(),
                true
            );
        }
    }
