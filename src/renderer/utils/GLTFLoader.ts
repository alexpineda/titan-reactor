    ...
    loadTexture( textureIndex: number ) {
        ...
        if ( !loader ) {
            if (
                ...
                json.extensionsRequired.includes( this.name )
            ) {
                throw new Error(
...
class GLTFParser {
    ...
    constructor( json: GLTFJSON, options = {} ) {
        ...
        const firefoxVersion =
            navigator.userAgent.match( /Firefox\/([0-9]+)\./ )?.[1] ?? -1;
    ...
    getDependency(
        ...
        Object3D | Material | THREE.Texture | AnimationClip | ArrayBuffer | object
    > {
            ...
            switch ( type ) {
                ...
                case "mesh":
                    dependency = this._invokeOne( function ( ext: {
                        loadMesh?: ( index: number ) => Promise<Mesh>;
                    } ) {
    ...
    loadImageSource( sourceIndex: number, loader: TextureLoader | ImageBitmapLoader ) {
        ...
        if ( sourceDef.bufferView !== undefined ) {
            ...
            sourceURI = this.getDependency( "bufferView", sourceDef.bufferView ).then(
                ( bufferView ) => {
                    isObjectURL = true;
    ...
    loadNode( nodeIndex: number ) {
        ...
        return ( () => {
            ...
            if ( nodeDef.camera !== undefined ) {
                pending.push(
                    this.getDependency( "camera", nodeDef.camera ).then( function (
                        camera
                    ) {
                        return this._getNodeRef(
                            this.cameraCache,
                            nodeDef.camera,
                            camera
                        );
