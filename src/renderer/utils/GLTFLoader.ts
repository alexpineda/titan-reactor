...
import { GLTFJSON } from "./GLTF";

const navigator = window.navigator ?? { userAgent: "" };

type Fn = (...args: any[]) => any;
type Dependency = "scene" | "node" | "mesh" | "accessor" | "bufferView" | "buffer" | "material" | "texture" | "skin" | "animation" | "camera";
...
type PluginCallback = (parser: GLTFParser) => GLTFLoaderPlugin;
type OnLoadCallback = (gltf: GLTF) => void;
type OnErrorCallback = (event: unknown) => void;
...
class GLTFLoader extends Loader {
    ...
    load(url: string, onLoad: OnLoadCallback, onProgress: Fn, onError: OnErrorCallback) {
        ...
        loader.load(url, (data) => {
            ...
            try {
                ...
                this.parse(data, resourcePath, (gltf: GLTF) => {
    ...
    register(callback: PluginCallback) {
    ...
    unregister(callback: PluginCallback) {
    ...
    parse(data: ArrayBuffer | string, path: string, onLoad: OnLoadCallback, onError: OnErrorCallback) {
...
function GLTFRegistry() {
    ...
    return {
        ...
        add: function (key: string, object: unknown) {
 ...
 */
class GLTFLightsExtension implements GLTFLoaderPlugin {

    name: string;
    parser: GLTFParser;
    cache: { refs: object, uses: object };
    ...
    _loadLight(lightIndex: number): Promise<DirectionalLight | PointLight | SpotLight> {
        ...
        const extensions = this.parser.json?.extensions?.[this.name] ?? {};
 ...
 */
class GLTFMaterialsUnlitExtension implements GLTFLoaderPlugin {
 ...
 */
class GLTFMaterialsEmissiveStrengthExtension implements GLTFLoaderPlugin {
 ...
 */
class GLTFMaterialsClearcoatExtension implements GLTFLoaderPlugin {
 ...
 */
class GLTFMaterialsIridescenceExtension implements GLTFLoaderPlugin {
 ...
 */
class GLTFTextureBasisUExtension implements GLTFLoaderPlugin {
 ...
 */
class GLTFTextureWebPExtension implements GLTFLoaderPlugin {
 ...
 */
class GLTFMeshoptCompression implements GLTFLoaderPlugin {
...
class GLTFBinaryExtension implements GLTFLoaderPlugin {
...
class GLTFDracoMeshCompressionExtension {
    ...
    decodePrimitive(primitive, parser: GLTFParser) {
...
class GLTFTextureTransformExtension {
    ...
    extendTexture(texture: Texture, transform) {
...
class GLTFMaterialsPbrSpecularGlossinessExtension implements GLTFLoaderPlugin {
 ...
 */
class GLTFMeshQuantizationExtension implements GLTFLoaderPlugin {
 ...
 */
function assignExtrasToUserData(object: Object3D | Material | BufferGeometry, gltfDef) {
 ...
 */
function updateMorphTargets(mesh: Mesh, meshDef) {
...
function getNormalizedComponentScale(constructor: Int8ArrayConstructor | Uint8ArrayConstructor | Int16ArrayConstructor | Uint16ArrayConstructor): number {
...
class GLTFParser {
    ...
    constructor(json: GLTFJSON, options = {}) {
        ...
        const firefoxVersion = navigator.userAgent.match(/Firefox\/([0-9]+)\./)?.[1] ?? - 1;
    ...
    parse(onLoad: OnLoadCallback, onError: OnErrorCallback) {
     ...
     */
    _addNodeRef(cache, index: number) {
    ...
    _getNodeRef(cache, index: number, object) {
     ...
     */
    getDependency(type: Dependency, index: number): Promise<Object3D | Material | THREE.Texture | AnimationClip | ArrayBuffer | object> {
        ...
        if (!dependency) {
            ...
            switch (type) {
                ...
                case "mesh":
                    dependency = this._invokeOne(function (ext: { loadMesh?: (index: number) => Promise<Mesh> }) {
     ...
     */
    getDependencies(type: Dependency) {
    ...
    // unsure on loader type
    loadTextureImage(textureIndex: number, sourceIndex: number, loader: TextureLoader | ImageBitmapLoader): Promise<Texture> {
    ...
    // unsure on loader type
    loadImageSource(sourceIndex: number, loader: TextureLoader | ImageBitmapLoader) {
     ...
     */
    assignTexture(materialParams: MeshStandardMaterialParameters, mapName: string, mapDef, encoding?: string): Promise<Texture> {
    ...
    loadMaterial(materialIndex: number) {
        ...
        if (materialDef.emissiveTexture !== undefined && materialType !== MeshBasicMaterial) {
            ...
            pending.push(this.assignTexture(materialParams, "emissiveMap", materialDef.emissiveTexture, sRGBEncoding));
...
function buildNodeHierarchy(nodeId: number, parentObject: Object3D, json: GLTFJSON, parser: GLTFParser) {
 ...
 */
function computeBounds(geometry: BufferGeometry, primitiveDef, parser: GLTFParser) {
 ...
 */
function addPrimitiveAttributes(geometry: BufferGeometry, primitiveDef, parser: GLTFParser) {
