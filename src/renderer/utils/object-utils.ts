...
export function borrow<
    ...
    TKeepRefs extends boolean = false
>( source: T, userOptions: BorrowOptions<TKeepRefs> = {} ): Borrowed<T, TKeepRefs> {
    const opts = {
        target: {},
        refRoot: true,
        retainGetters: false,
...
export function expose<T extends object, K extends keyof T>(
    source: T,
    keys: K[],
    { asValues }: ExposeOptions = { asValues: false }
): Pick<T, K> {
...
export const arrayOverwriteMerge = ( _: any, sourceArray: unknown[] ) => sourceArray;
