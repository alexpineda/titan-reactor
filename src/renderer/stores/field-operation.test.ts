...
describe("macroEffectApplyNumeric", () => {
    ...
    it("should set value to default", () => {
        const effect = fieldOperation(Operator.SetToDefault, { value: 0 }, undefined, 2);
        expect(effect).toBe(2);
    });
    ...
    it.each([
        ...
        const effectResult = fieldOperation(Operator.Increase, field, undefined, undefined);
    ...
    it.each([
        ...
        const effectResult = fieldOperation(Operator.Decrease, field, undefined, undefined);
        expect(effectResult).toBe(expected);
    });
    ...
    it.each([
        ...
        const effectResult = fieldOperation(Operator.IncreaseCycle, field, undefined, undefined);
        expect(effectResult).toBe(expected);
    });
    ...
    it.each([
        ...
        const effectResult = fieldOperation(Operator.DecreaseCycle, field, undefined, undefined);
        expect(effectResult).toBe(expected);
    });
...
describe("macroEffectApplyList", () => {
    ...
    it("should set value", () => {
        const effect = fieldOperation(Operator.Set, { value: "a", options: ["a", "b"] }, "b", undefined);
        expect(effect).toBe("b");
    });

    it("should not set value if option does not exist", () => {
        const effect = fieldOperation(Operator.Set, { value: "a", options: ["a", "b"] }, "c", undefined);
        expect(effect).toBe("a");
    });

    it("should set 0th option if field.value is invalid", () => {
        const effect = fieldOperation(Operator.Set, { value: "c", options: ["a", "b"] }, null, undefined);
        expect(effect).toBe("a");
    });

    it("should accept object option types", () => {
        const effect = fieldOperation(Operator.Set, { value: "a", options: { "Letter A": "a", "Letter B": "b" } }, "b", undefined);
        expect(effect).toBe("b");
    });

    it("should increase the value", () => {
        const effect = fieldOperation(Operator.Increase, { value: "a", options: ["a", "b"] }, null, undefined);
        expect(effect).toBe("b");
    });


    it("should clamp the increased the value", () => {
        const effect = fieldOperation(Operator.Increase, { value: "b", options: ["a", "b"] }, null, undefined);
        expect(effect).toBe("b");
    });

    it("should decrease the value", () => {
        const effect = fieldOperation(Operator.Decrease, { value: "b", options: ["a", "b"] }, null, undefined);
        expect(effect).toBe("a");
    });


    it("should clamp the decreased the value", () => {
        const effect = fieldOperation(Operator.Decrease, { value: "a", options: ["a", "b"] }, null, undefined);
        expect(effect).toBe("a");
    });

    it("should cycle the increased the value", () => {
        const effect = fieldOperation(Operator.IncreaseCycle, { value: "b", options: ["a", "b"] }, null, undefined);
        expect(effect).toBe("a");
    });

    it("should cycle the decreased the value", () => {
        const effect = fieldOperation(Operator.DecreaseCycle, { value: "a", options: ["a", "b"] }, null, undefined);
        expect(effect).toBe("b");
    });

    it("should set max", () => {
        const effect = fieldOperation(Operator.Max, { value: "a", options: ["a", "b", "c"] }, null, undefined);
        expect(effect).toBe("c");
    });

    it("should set min", () => {
        const effect = fieldOperation(Operator.Min, { value: "c", options: ["a", "b", "c"] }, null, undefined);
        expect(effect).toBe("a");
    });
