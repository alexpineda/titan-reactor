...
export const speedHandler = (direction: SpeedDirection, _openBW: WeakRef<OpenBW>) => {
    ...
    } else if (direction === SpeedDirection.Down && currentSpeed <= 2 && currentSpeed > 1) {
        newSpeed = currentSpeed - REPLAY_MIN_SPEED;
    } else {
        newSpeed = Math.max(REPLAY_MIN_SPEED, Math.min(REPLAY_MAX_SPEED, currentSpeed * (SpeedDirection.Up === direction ? 2 : 0.5)));
    }
