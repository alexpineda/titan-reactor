...
export const createOpenBW = async () => {
    ...
    openBW.setPlayerVisibility = ( visibility: number ) =>
        wasm._set_player_visibility( visibility );
    ...
    openBW.loadReplayWithHeightMap = (
        buffer: Buffer,
        ...
        tryCatch( () => {
            ...
            wasm._load_replay_with_height_map(
                replayBuf,
                buffer.length,
                heightMapBuf,
                data.length,
    ...
    openBW.start = async ( readFile: ReadFile ) => {
        ...
        const files = new OpenBWFileList( wasm );
