...
import { CloseCascStorage, OpenCascStorage, ReadCascFile, ReadCascFileBatch } from "common/types";
...
export const readCascFile: ReadCascFile = async (filepath: string, encoding?: BufferEncoding) => {
  const arrayBuffer = await ipcRenderer.invoke(OPEN_CASCLIB_FILE, filepath, encoding);
  // return Buffer.from(arrayBuffer.buffer);
  return arrayBuffer;
};

export const readCascFileBatch: ReadCascFileBatch = async (filepaths: string[], encoding?: BufferEncoding) => {
