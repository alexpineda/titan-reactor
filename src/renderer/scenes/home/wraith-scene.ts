...
const createWraith = (og: Object3D, originalPosition: Vector3) => {
    ...
    const rightBlinker = new PointLight(0xff0000, 2, 1, 7);
    rightBlinker.position.set(-0.2, -0.2, -0.05);
    wraith.add(rightBlinker);

    const leftBlinker = new PointLight(0x00ff00, 2, 1, 7);
...
const INTRO_LOOP = async (elapsed: number) => {
    ...
    _fireParticles.update(camera, delta);
...
export const preloadIntro = async () => {
    const { increment, complete } = processStore().start(Process.AtlasPreload, 5);
    //@ts-ignore
    ...
    const { model: wraith } = await loadGlb(`${__static}/wraith.glb`, envmap);
    increment();
    ...
    _asteroidInstances = new InstancedMesh(
    ...
    _fireParticles = createParticles({
        count: 5000,
        size: 0.03,
        coordScale: 0.04,
        tex: fireTexture,
        particleTemplate: (opts: ParticleSystemOptions) => {
            const x = THREE.MathUtils.randFloatSpread(0.5) * opts.coordScale;
            const y = THREE.MathUtils.randFloatSpread(0.5) * opts.coordScale
            const z = THREE.MathUtils.randFloatSpread(0.5) * opts.coordScale;

            const position = new THREE.Vector3(x, y, z);

            const life = THREE.MathUtils.randFloat(0.05, 0.6);
            const size = THREE.MathUtils.randFloat(0.8, 1) * 4.0;

            return {
                position,
                size,
                currentSize: size,
                color: new THREE.Vector4(1, 0.5, 0.1, THREE.MathUtils.randFloat(0.5, 1)),
                life,
                maxLife: life,
                angle: 0,
                velocity: new THREE.Vector3(0, 0, -35000 * opts.coordScale),
                frame: 0,
                maxFrame: 64
            };
        }
...
let _fireParticles: ReturnType<typeof createParticles>;
...
export async function createWraithScene() {
    ...
    window._fireParticles = _fireParticles;
