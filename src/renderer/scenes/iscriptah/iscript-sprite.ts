...
import { drawFunctions, imageTypes, iscriptHeaders, overlayTypes } from "common/enums";
...
export class IScriptSprite extends Group {
    ...
    createTitanImage: (image: number) => ImageBase;
    ...
    constructor(
        ...
        bwDat: BwDAT,
        createSprite: (unit: UnitDAT | null | undefined) => IScriptSprite,
        createImage: (image: number) => ImageBase,
        runner: IScriptRunner,
        createSpriteCb: (titanSprite: IScriptSprite) => void,
    ...
    spawnIScriptImage(imageIndex: number, imageOrder?: ImageOrder, rel?: number): any {
        const relImage =
            rel === undefined ? this.iscriptImages.indexOf(this.mainImage!) : rel;
        let pos = relImage;
        ...
        const state = new IScriptState(
            this.bwDat.iscript.iscripts[image.dat.iscript],
            image.dat
        );
    ...
    setDirection(direction: number) {
        ...
        this.runner.setFrame(
            this.mainImage.state.frame,
            this.mainImage.state.flip,
    ...
    update(delta: number, cameraDirection: number) {
        ...
        if (_terminated) {
            this.iscriptImages = this.iscriptImages.filter(
                (image) => !image.state.terminated
            );
                ...
                case "imgul":
                    {
                        ...
                        const spawnedImage = this.spawnIScriptImage(
                            imageId,
                            ImageOrder.below
                        );
                ...
                case "sprol":
                    {
                        ...
                        sprite.spawnIScriptImage(
                            this.bwDat.sprites[spriteId].image.index
                        );
                ...
                case "creategasoverlays":
                    {
                        ...
                        const los =
                            this.bwDat.los[
                                iscriptImage.state.imageDesc.specialOverlay - 1
                            ];
                ...
                case "followmaingraphic":
                    {
                        if (this.mainImage) {
                            ...
                            iscriptImage.image.setFrame(
                                iscriptImage.state.frame,
                                iscriptImage.state.flip
                            );
                        }
                    }
                    break;
    ...
    _getImageLoOffset(
        ...
        const frame = useFrameset
            ? iscriptImage.state.frameset
            : iscriptImage.state.frame;
