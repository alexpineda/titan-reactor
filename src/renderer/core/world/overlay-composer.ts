...
export const createOverlayComposer = (
    ...
    surfaces: Borrowed<SurfaceComposer>,
    views: ViewInputComposer,
    ...
    world.events.on( "box-selection-start", () =>
        visualBox.start( views.inputs.mouse.clientX, views.inputs.mouse.clientY )
    );

    world.events.on( "box-selection-move", () =>
        visualBox.end( views.inputs.mouse.clientX, views.inputs.mouse.clientY )
    );
    ...
    return {
        ...
        update( delta: number ) {
            ...
            cursorMaterial.update(
                delta,
                views.inputs.mouse.move,
                _insideMinimap ? UnitSelectionStatus.None : unitSelectionBox.status
            );
            ...
            if ( _intersects.length && _intersects[0].uv ) {
                ...
                uv.set(
                    ( uv.x - bounds.x ) / mapAspect.y,
                    ( uv.y - bounds.y ) / mapAspect.x
                );

                if ( views.inputs.mouse.move.z > -1 ) {
                    views.onMinimapDragUpdate(
                        uv
                            .set( uv.x, 1 - uv.y )
                            .subScalar( 0.5 )
                            .multiply( mapV ),
                        !!views.inputs.mouse.clicked,
                        views.inputs.mouse.move.z
                    );
                }
            }
        },
