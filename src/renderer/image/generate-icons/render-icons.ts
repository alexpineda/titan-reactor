...
export const renderIconsToBlob = function* (
    ...
    for ( const _dds of dds ) {
        const texture = createDDSTexture( parseDDS( _dds ) );
        ...
        if ( alpha ) {
            ...
            yield new Promise<Blob | null>( ( res ) =>
                alphaCanvas.toBlob( res, "image/png" )
            );
        } else {
            yield new Promise<Blob | null>( ( res ) => canvas.toBlob( res, "image/png" ) );
        }
    }
};
