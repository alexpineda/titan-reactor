...
export const loadAllTilesetData = async ( tileset: number ) => {
    const tilesets = [
        "badlands",
        "platform",
        "install",
    ...
    const tilegroupsCV5 = new Uint16Array(
        ( await readCascFile( `TileSet/${tilesetName}.cv5` ) ).buffer
    );
    ...
    const creepGrpHD = await readCascFile( `TileSet/${tilesetName}.dds.grp` );
    ...
    const waterMask = noWaterMasks.includes( tileset )
        ? null
        : await readCascFile( `TileSet/${tilesetName}_mask.dds.grp` );
    const tileMask = noWaterMasks.includes( tileset )
        ? null
        : await readCascFile( `TileSet/${tilesetName}.tmsk` );
