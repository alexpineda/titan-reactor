...
class Face {
    ...
    set(pos: BufferAttribute | InterleavedBufferAttribute, i0: number, i1: number, i2: number) {
    ...
    get equalZ() {
        return this.points[0].z === this.points[1].z && this.points[1].z === this.points[2].z;
    }
}
...
class Tile {
    ...
    get equalZ() {
        return this.face1!.equalZ && this.face2!.equalZ && this.face1!.centerZ === this.face2!.centerZ;
    }
...
const computeMergeTiles = (tiles: Tile[][]) => {
    ...
    for (let x = 0; x < tiles.length; x++) {
        for (let y = 0; y < tiles[x].length; y++) {
            ...
            let i = 1;
            while (tiles[x + i] && tiles[x + i][y] !== undefined && tiles[x + i][y].equalZ && tiles[x + i][y].centerZ === tile.centerZ && tiles[x + i][y].merged === false) {
                i++;
            }
