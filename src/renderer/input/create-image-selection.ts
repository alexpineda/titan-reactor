...
export const createImageSelection = (scene: Scene, gameSurface: Surface, minimapSurface: Surface, onSelect: (obj: Object3D[]) => void) => {
    const janitor = new Janitor("createImageSelection");
    ...
    janitor.addEventListener(gameSurface.canvas, 'pointerup', 'pointerup', _selectUp);
    janitor.addEventListener(gameSurface.canvas, "pointerdown", "pointerdown", _selectDown);
    janitor.addEventListener(gameSurface.canvas, 'pointermove', 'pointermove', _selectMove);
