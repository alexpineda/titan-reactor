...
export const defaultUpdate =
    ( {
        ...
        alpha: FnOrValue<number>;
        size: FnOrValue<number>;
        velocity: Vector3;
    } ) =>
    ( t: number, delta: number, p: Particle, opts: ParticleSystemDefinition ) => {
        ...
        if ( opts.spriteMap ) {
            p.frame =
                Math.floor( opts.spriteMap.loop * t * opts.spriteMap.frameCount ) %
                opts.spriteMap.frameCount;
        }
    };
...
export function createParticles<T extends ParticleSystemDefinition>( _opts: T ) {
    ...
    const material = new ShaderMaterial( {
        ...
        uniforms: {
            ...
            uFrame: {
                value: new Vector2(
                    opts.spriteMap?.width ?? 1,
                    opts.spriteMap?.height ?? 1
                ),
            },
        },
    } );
    ...
    const updateGeometry = () => {
        ...
        geom.setAttribute( "angle", new Float32BufferAttribute( angles, 1 ) );
        geom.setAttribute( "frame", new Float32BufferAttribute( frames, 1 ) );
    ...
    return {
        ...
        update( camera: THREE.PerspectiveCamera, delta: number ) {
            const d = delta / 1000;
            material.uniforms.scale.value = Math.abs(
                window.innerHeight /
                    ( 2 * Math.tan( MathUtils.degToRad( camera.getEffectiveFOV() * 0.5 ) ) )
            );
